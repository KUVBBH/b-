...
def tqdm_run(q) :
    ...
    t=q[0]
    o=q[1]
    l=len(os.listdir(o))+t
    #print(len(os.listdir(o))+t)
    while True :
        T_2='%.2f'%(time.time()-T_1)
        h=len(os.listdir(o))
        print('\r','%.2f'%((t-(l-h))/t*100),'%','■'*int((t-(l-h))/t*10),'<',T_2,'>',end='')
        sys.stdout.flush()
        time.sleep(1)
...
>>>请输入:''').split('-')
    ...
    L=[len(ffmpeg_list),out_path]
    q=Queue()
    q.put(L)
    pr=Process(target=tqdm_run,daemon=True,args=(q,))
    pr.start()
    pool=Pool(processes = int(os.cpu_count()-1))
    for i in ffmpeg_list :
        pool.apply_async(func=ffmpeg_run,args=(i,))
        #pool.apply(func=ffmpeg_run,args=(i,))
    pool.close()
    pool.join()
    print('\n'+'--------------已完成--------------')
